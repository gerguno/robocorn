...
export default function App() {
  ...
  return (
      <>
          ...
          {(answers.step === 6) && <Question content={`Сколько будет 7 минус 4`} variable={'eq1'}/>}
          {(answers.step === 7) && (
              (Number(answers.eq1) === 3) ? <Answer content={`Молодец! У меня для тебя есть песня`}/> : <Answer content={`Неправильно, подумай пожалуйста еще раз`} stepForward={false}/>
          )}
          {(answers.step === 8) && <Answer content={`Сейчас будет балдёж!`}/>}
          {(answers.step === 9) && <Song link={`https://www.youtube.com/watch?v=ADlGkXAz1D0`}/>}
      </>
  )
}
